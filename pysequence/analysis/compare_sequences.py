...
def intersection(list1, list2):
    """Returns a list containing elements which appear in list1 and list2"""
    result = [value for value in list1 if value in list2]
    return result

def get_intersection_of_sequences(n, seq_name1, seq_name2):
    """Returns a list of numbers below n appearing in two nams sequences"""
...
if __name__ == "__main__":
    n = 50
    fibonnaci_primes = get_intersection_of_sequences(n, 'prime', 'fibonacci')
    n_fibonnaci_primes = len(fibonnaci_primes)
    print(f'There are {n_fibonnaci_primes} Fibonacci primes below {n}. These are:')
    for p in fibonnaci_primes:
        print(f'   {p}')